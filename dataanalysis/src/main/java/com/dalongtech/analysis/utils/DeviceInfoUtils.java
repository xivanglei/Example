...
public class DeviceInfoUtils {
    ...
    public static Map<String, String> getDeviceInfo(Context context) {
        if (mDeviceInfo == null) {
            ...
            deviceInfo.put("$android_lib_version", Constants.DEV_SDK_VERSION);
    ...
    public static String getDeviceID(Context context) {
        ...
        String id = SharedUtil.getString(context, ID_KEY, null);
        if (!CommonUtils.isEmpty(id)) {
            ...
            id = getIMEI(context);
            if (!CommonUtils.isEmpty(id)) {
                SharedUtil.setString(context, ID_KEY, id);
                return id;
            }
            // 通过AndroidID获取ID
    ...
    private static String getSerialNum(Context context) {
        if (Build.VERSION.SDK_INT < 26) {
            ...
            if (CommonUtils.checkPermission(context, Manifest.permission.READ_PHONE_STATE)) {
                // 反射获取
                try {
    ...
    private static String generateID(Context context) {
        String key = CommonUtils.getAppKey(context);
