...
public class LogPrompt {
    ...
    public static void showCheckTimeLog(long netTime, long nowTime, long absDiff) {
        MMLog.d("收到服务器的时间：" + CommonUtils.timeConversion(netTime));
        MMLog.d("本地时间：" + CommonUtils.timeConversion(nowTime));
        MMLog.d("时间相差：" + (absDiff / 1000) + " 秒，数据将会进行时间校准。");
    }
    ...
    public static void showLog(String apiName) {
        if (!TextUtils.isEmpty(apiName)) {
            ...
            MMLog.w(apiName + LogPrompt.FAILED);
        }
    }
    ...
    public static void showLog(String apiName, String logDetailed) {
        if (!TextUtils.isEmpty(apiName)) {
            // 日志格式：应用接口名称 + 结果 (+ 失败详细信息)
            MMLog.w(apiName + LogPrompt.FAILED + logDetailed);
        }
    }

    public static void encryptLog(boolean success) {
        if (success) {
            MMLog.d(ENCRYPT_SUCCESS);
        } else {
            MMLog.w(ENCRYPT_FIELD);
        }
    }
    ...
    public static void showLog(String apiName, boolean success) {
        ...
        if (success) {
            MMLog.d(apiName + SUCCESS);
        } else {
            MMLog.w(apiName + FAILED);
        }
    }

    public static void showInitLog(boolean success) {
        if (success) {
            MMLog.d(INIT_SUCCESS + Constants.DEV_SDK_VERSION);
        } else {
            MMLog.w(INIT_FAILED);
        }
    }
    ...
    public static void showChannelLog(boolean success, String channel) {
        if (success) {
            MMLog.d(Constants.API_INIT + CHANNEL_SUCCESS + channel);
        } else {
            MMLog.w(Constants.API_INIT + CHANNEL_FAILED + channel);
        }
    }

    public static void showCAgentLog(boolean success, String cAgent) {
        if (success) {
            MMLog.d(Constants.API_INIT + C_AGENT_SUCCESS + cAgent);
        } else {
            MMLog.w(Constants.API_INIT + C_AGENT_FAILED + cAgent);
        }
    }

    public static void showPartnerCodeLog(boolean success, String partnerCode) {
        if (success) {
            MMLog.d(Constants.API_INIT + PARTNER_CODE_SUCCESS + partnerCode);
        } else {
            MMLog.w(Constants.API_INIT + PARTNER_CODE_FAILED + partnerCode);
        }
    }
    ...
    public static void showKeyLog(boolean success, String key) {
        if (success) {
            ...
            MMLog.w(Constants.API_INIT + KEY_FAILED + key);
        }
    }
    ...
    public static void showSendMessage(String url, JSONArray data) {
        MMLog.d("Send message to server:" + url + "\n data:  " + data);
    }
    ...
    public static void showSendResults(boolean isSuccess) {
        if (isSuccess) {
            MMLog.d("Data uploaded successfully.");
        } else {
            MMLog.w("Data uploaded failed.");
        }
    }
}