...
import com.dalongtech.magicmirror.process.LogBean;
...
public class ParameterCheck {
     ...
     */
    public static int checkKey(Object objKey) {
        try {
     ...
     * 校验value数据类型（配置）
     */
    public static int checkValue(Object value) {
        try {
    ...
    private static boolean isReservedKeywords(String key) {
        for (int i = 0; i < reservedKeywords.length; i++) {
            if (key.equals(reservedKeywords[i])) {
                return false;
            }
        }
        return true;
